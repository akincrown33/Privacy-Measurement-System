...
numberOfPossibleValues = [3,3,2,2];                                         % variables for Laplace smoothing


%% Training

% Initial variables
numVar = size(trainingSet,2)-1; classes = unique(trainingSet(:,end));
numClasses = size(classes,1); P=cell(numClasses,numVar);
...
numRows=size(dataSet,1); probabilities=ones(1,numClasses);
